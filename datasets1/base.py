...
class Dataset(Dataset):
    ...
    def __getitem__(self, index):
        if self.condition == 1:
            # condition
            img0 = Image.open(self.gt[index]).convert('L')
            img1 = Image.open(self.input[index]).convert('L')
            w, h = img0.size
            ...
            if self.crop_patch and not self.sample:
                img0, img1 = self.get_patch([img0, img1], self.image_size)#不执行

            img1 = self.cv2equalizeHist(img1) if self.equalizeHist else img1#不执行
