...
class ResnetBlock(nn.Module):
    def __init__(self, dim, dim_out, *, time_emb_dim=None, groups=8):
        ...
        # self.block3 = Block(dim, dim_out, groups=groups)
        # self.sig = nn.Sigmoid()

        self.res_conv = nn.Conv2d(
            dim, dim_out, 1) if dim != dim_out else nn.Identity()
        # self.CA = ChannelAttention(dim_out, 16)
...
class BlockFFT(nn.Module):
    ...
    def forward(self, x, scale_shift = None):
        ...
        x = x.imag * torch.view_as_complex(self.complex_weight)
        x = x.real + 1j * x
...
def tensor_to_binary(tensor, total_bits=32, fractional_bits=16):
    """
    将一个 B*C*W*H 的 Tensor 转换为二进制表示，每个数字用 total_bits 表示。

    参数：
    - tensor: 输入的 Tensor，形状为 B*C*W*H。
    - total_bits: 总位数，包括符号位、整数位和小数位。
    - fractional_bits: 小数部分的位数。

    返回：
    - binary_tensor: 输出的 Tensor，形状为 B*C*W*H*total_bits。
    """
    # 处理符号位
    sign_bit = (tensor < 0).float()  # 负数的符号位为 1，正数为 0

    # 取绝对值
    abs_tensor = torch.abs(tensor)

    # 分离整数和小数部分
    integer_part = abs_tensor.long()  # 整数部分
    fractional_part = abs_tensor - integer_part.float()  # 小数部分

    # 转换整数部分为二进制
    integer_binary = []
    for i in range(total_bits - fractional_bits - 1):  # 整数部分位数（去掉符号位和小数位）
        integer_binary.append((integer_part % 2).float())
        integer_part = integer_part // 2
    integer_binary = torch.stack(integer_binary, dim=-1).flip(-1)  # 翻转为正确顺序

    # 转换小数部分为二进制
    fractional_binary = []
    for i in range(fractional_bits):
        fractional_part = fractional_part * 2
        fractional_binary.append(fractional_part.floor())
        fractional_part = fractional_part - fractional_part.floor()
    fractional_binary = torch.stack(fractional_binary, dim=-1)

    # 合并符号位、整数部分和小数部分
    full_binary = torch.cat([sign_bit.unsqueeze(-1), integer_binary, fractional_binary], dim=-1)

    # 调整形状为 B*C*W*H*total_bits
    binary_tensor = full_binary.reshape(*tensor.shape, total_bits)
    binary_tensor = torch.transpose(binary_tensor, 1, 3)
    binary_tensor = torch.transpose(binary_tensor, 2, 3)
    return binary_tensor

class Blockfft(nn.Module):#加入的模块
    # def __int__(self, dim, h, w):
    def __init__(self):
        super().__init__()
        ...
        channels = 1
        b_k = partial(ResnetBlock, groups=resnet_block_groups)
        ...
        for ind, (dim_in, dim_out) in enumerate(in_out):
            ...
            self.downs.append(nn.ModuleList([
                b_k(dim_in, dim_in, time_emb_dim=time_dim),
                b_k(dim_in, dim_in, time_emb_dim=time_dim),
    ...
    def forward(self, x ,t):
        ...
        x =np.fft.fft2(x,axes=(2,3), norm='ortho')
        ...
        x1 = x1.squeeze(0)
        x1 = tensor_to_binary(x1, total_bits=64, fractional_bits=32)
        x2 = torch.transpose(x2,0,1)
        x2 = x2.squeeze(0)
        x2 = tensor_to_binary(x2, total_bits=64, fractional_bits=32)
        # x1 = (1 - torch.exp(-0.005 * x1))/(1 + torch.exp(-0.005 * x1))
        # x2 = (1 - torch.exp(-0.005 * x2)) / (1 + torch.exp(-0.005 * x2))
        #
        ...
        for block1, block2, attn, downsample in self.downs:
            ...
            x = attn(x)
            x2 = downsample(x)
        x1 = torch.fft.irfft2(x1, s=(32, 32), dim=(2, 3), norm='ortho')
        x2 = torch.fft.irfft2(x2, s=(32, 32), dim=(2, 3), norm='ortho')
...
class Unet(nn.Module):
    def __init__(
        ...
        init_dim = default(init_dim, dim)
        self.init_conv = nn.Conv2d(input_channels, init_dim, 7, padding=3)
        # self.conv1 = nn.Conv2d(1536, 512, kernel_size =1)
    ...
    def forward(self, x, time, x_input, x_self_cond=None):
        ...
        x1, x2 = self.blockfft(x_input,t)
        x = x + self.decouple(x) +x1+x2
        # x = x + self.decouple(x)
...
class UnetRes(nn.Module):
    ...
    def forward(self, x, time, x_input, x_self_cond=None):
        if self.share_encoder == 1:
            ...
            return self.unet0(x, time, x_input, x_self_cond=x_self_cond), self.unet1(x, time, x_input, x_self_cond=x_self_cond)
        elif self.share_encoder == -1:
            return [self.unet0(x, time, x_input, x_self_cond=x_self_cond)]
...
class sup_reso(nn.Module):
    def __init__(self):
        super().__init__()

        self.conv1 = nn.ConvTranspose2d(1, 64, 2, 2)
...
class ResidualDiffusion(nn.Module):
    ...
    # Sobel边缘损失函数
    # def SobelLoss(self, input_tensor, target_tensor):
    ...
    #     # 计算输入和目标图像的Sobel梯度
    #     input_grad_x = F.conv2d(input_tensor, sobel_x, padding=1, groups=input_tensor.size(1))
    #     input_grad_y = F.conv2d(input_tensor, sobel_y, padding=1, groups=input_tensor.size(1))
    #     target_grad_x = F.conv2d(target_tensor, sobel_x, padding=1, groups=target_tensor.size(1))
    #     target_grad_y = F.conv2d(target_tensor, sobel_y, padding=1, groups=target_tensor.size(1))
    #
    #     # 计算Sobel梯度的差异
    #     sobel_loss = F.l1_loss(input_grad_x, target_grad_x) + F.l1_loss(input_grad_y, target_grad_y)
    #
    #     return sobel_loss
    ...
    def p_losses(self, imgs, t, noise=None):
        ...
        # maybe_clip = partial(torch.clamp, min=-1.,
        #                      max=1.)
        # pred_res = model_out1[0]
        # pred_noise = model_out1[1]
        # pred_res = maybe_clip(pred_res)
        # x_start1 = self.predict_start_from_res_noise(
        #     x, t, pred_res, pred_noise)
        # x_start1 = maybe_clip(x_start1)
        #
        # x_reso = self.supr(x_start1)
        # x_start2 = x_start.squeeze(0)
        # x_start2 = x_start2.squeeze(0)
        # x_start2 = x_start2.cpu().numpy()
        # x_start2 = cv2.resize(x_start2, (512, 512))
        # x_start2 = torch.from_numpy(x_start2).cuda()
        #
        # x_start2 = x_start2.unsqueeze(0)
        # x_start2 = x_start2.unsqueeze(0)
        #
        # loss_reso = self.loss_fn(x_reso,x_start2)
        ...
        if u_loss:
            ...
            for i in range(len(model_out)):
                # sobel = self.SobelLoss(model_out[i], target[i])
                loss = loss + \
                    alpha * self.loss_fn(model_out[i], target[i], reduction='none') + 0.1 *self.loss_fn(model_out[i], target[i], reduction='none')
                # loss = loss +loss_reso
...
class Trainer(object):
    def __init__(
        ...
        *,
        ...
        num_samples=25,
        results_folder='./results/sample2',
        amp=False,
        fp16=False,
        split_batches=True,
    ...
    def save(self, milestone):
        ...
        # data2 = self.model.supr.state_dict()

        torch.save(data, str(self.results_folder / f'model-{milestone}.pt'))
        # torch.save(data2, str(self.results_folder / f'model_reso-{milestone}.pt'))
    ...
    #                             k += 1
    #             all_images = torch.cat(all_images_list, dim=0)
    #             # path = Path('/home/zhoutao/tmp/RDDM-main/RDDM-main/experiments/testcode/results/sample1/model_reso-2.pt')
    #             # model2 = self.model.supr
    #             # loaded_module2_weights = torch.load(path)
    #             # model2.load_state_dict(loaded_module2_weights)
    #             # output = model2(all_images)
    #             if last:
    #                 nrow = int(math.sqrt(self.num_samples))
    #             else:
    #                 nrow = all_images.shape[0]

    #             # utils.save_image(output, str(
    #             #     self.results_folder / file_name1), nrow=nrow)
    #             utils.save_image(all_images, str(
    #                 self.results_folder/file_name1), nrow=nrow)
    #             print("test-save "+file_name1)
    #     else:
    #         if FID:
    ...
    def count_parameters(self, model=None):
        if model is None:
        # 只统计真正的 denoise 网络
            model = self.ema.ema_model.model if hasattr(self.ema.ema_model, 'model') else self.ema.ema_model
    ...
    def profile_flops(self, items=None):
        from torch.profiler import profile, ProfilerActivity

        diffusion = self.ema.ema_model
        model = diffusion.model if hasattr(diffusion, 'model') else diffusion
        model = model.to(self.device).eval()

        with torch.no_grad():
        # -------------------------
        # 1) 构造一次真实的单步 denoise 输入
        # -------------------------
            if self.condition:
                if self.condition_type == 1:
                # items: [B, C, H, W]，只有 input
                    x_input = items.to(self.device)
                ...
                # items: [gt, input, mask/extra_cond]
                    batch = [item.to(self.device) for item in items]
                    x_input = batch[1]
                    x_input_condition = batch[2]
                    x = torch.randn_like(x_input)

                else:
                    raise ValueError(f"Unknown condition_type: {self.condition_type}")

                if diffusion.input_condition:
                    x_in = torch.cat((x, x_input, x_input_condition), dim=1)
                else:
                    x_in = torch.cat((x, x_input), dim=1)
            ...
            # unconditional
                ...
                h = diffusion.image_size
                w = diffusion.image_size
                x = torch.randn(b, c, h, w, device=self.device)
                x_input = x
                x_in = x

            t = torch.randint(
                0, diffusion.num_timesteps, (x_in.shape[0],),
        ...
        # -------------------------
            ...
            with profile(
                ...
                _ = model(x_in, t, x_input, x_self_cond)
        ...
        # -------------------------
            ...
            print(f"Single-step FLOPs: {total_flops:,} ({total_flops / 1e9:.4f} GFLOPs)")

            sampling_steps = diffusion.sampling_timesteps if diffusion.is_ddim_sampling else diffusion.num_timesteps
            full_sampling_flops = total_flops * sampling_steps

            print(f"Sampling steps: {sampling_steps}")
            print(f"Estimated full sampling FLOPs: {full_sampling_flops:,} ({full_sampling_flops / 1e12:.4f} TFLOPs)")

            print("Note: torch.profiler with_flops mainly estimates supported ops like matmul/Conv2d; "
                "custom ops / FFT / some attention-related ops may be undercounted.")

            return total_flops, full_sampling_flops
    ...
    def test(self, sample=False, last=True, FID=False): 
        ...
        if self.condition:
            ...
            for items in loader:
                ...
                utils.save_image(all_images, str(self.results_folder / file_name1), nrow=nrow)
                ...
                print(
                    f"test-save {file_name1} | "
                    f"sample_time: {sample_end - sample_start:.4f}s | "
                    f"save_time: {save_end - save_start:.4f}s | "
                    f"total_time: {single_end - single_start:.4f}s"
                )

        else:
            if not flops_profiled:
                self.profile_flops(items=None)
                flops_profiled = True
            ...
            for i in range(n_rounds):
                ...
                print(f"round {i} time: {round_end - round_start:.4f}s")

        total_test_end = time.perf_counter()
        print(f"test end | total test time: {total_test_end - total_test_start:.4f}s")
